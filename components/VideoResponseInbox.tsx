...
export default function VideoResponseInbox() {
  ...
  async function loadResponses() {
    ...
    if (!user) {
      setMessage("Please sign in to view your video responses.");
      setLoading(false);
      return;
    }
    ...
    const normalizedResponses: VideoReplyRow[] = ((data ?? []) as RawVideoReplyRow[]).map(
      (item) => {
  ...
  return (
    <section className="rounded-[2rem] bg-white p-5 shadow-sm ring-1 ring-slate-200">
      ...
      {message && (
        <div className="mb-4 rounded-2xl bg-blue-50 p-4 text-sm font-bold text-[#082f63]">
          {message}
        </div>
      )}

      {loading ? (
        <div className="rounded-[1.5rem] bg-slate-50 p-5 text-sm font-bold text-slate-500 ring-1 ring-slate-200">
          Loading video responses...
        </div>
      ) : responses.length === 0 ? (
        <div className="rounded-[1.5rem] bg-slate-50 p-5 text-sm leading-6 text-slate-600 ring-1 ring-slate-200">
          No video responses yet. When someone responds to one of your videos,
          it will show here.
        </div>
      ) : (
