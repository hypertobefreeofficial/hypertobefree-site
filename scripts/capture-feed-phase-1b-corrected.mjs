...
const outDir = resolve(process.cwd(), ".screenshots/feed-phase-1b-corrected");
...
<body>
  <h1>Phase 1B Corrected Contact Sheet</h1>
  <div class="grid">
    ${contactFiles
      .map(
...
await sheetPage.screenshot({
  path: resolve(outDir, "contact-sheet-corrected.png"),
  fullPage: true,
});
...
console.log(`Contact sheet: ${resolve(outDir, "contact-sheet-corrected.png")}`);

const mid390 = resolve(outDir, "mobile-mid-390.png");
try {
  const stats = statSync(mid390);
  console.log(`mobile-mid-390 modified: ${stats.mtime.toISOString()}`);
} catch {
  console.log("mobile-mid-390.png not found");
}
