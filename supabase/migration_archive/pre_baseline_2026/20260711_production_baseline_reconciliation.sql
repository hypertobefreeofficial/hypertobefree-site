-- HTBF Production Public Schema Baseline Reconciliation
-- REVIEW ONLY — generated from supabase/production_public_schema.sql
-- Source: production pg_dump (schema-only, public, no owners/privileges/data)
-- Purpose: reproduce HTBF application public schema on a clean Supabase preview branch
-- DO NOT apply to production (schema already exists there).
--
-- ORDERING: filename 20260711 ensures this runs BEFORE 20260712+ incremental migrations.
-- Must precede 20260816_journey_private_media_phase1.sql.
--
-- Intentionally excluded from this file:
--   * storage.buckets / storage.objects policies (separate storage introspection)
--   * auth.users triggers (handle_new_user_profile, create_welcome_inbox_message)
--   * pg_dump session settings, schema creation, owner/grant statements
--   * journey-private-media bucket (added by 20260816 phase 1)
--
-- After this baseline, repo migrations 20260712–20260724 should largely no-op.
-- Migrations NOT reflected in production (20260715, 20260717, 20260719) will still
-- apply additive changes on a preview branch unless explicitly skipped.
