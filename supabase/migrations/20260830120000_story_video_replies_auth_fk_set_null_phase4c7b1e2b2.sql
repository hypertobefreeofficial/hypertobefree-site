-- Phase 4C.7B.1E.2B.2: story_video_replies auth-user FK preservation hardening
...
--
-- ---------------------------------------------------------------------------
-- PRODUCTION PRE-MIGRATION READ-ONLY CHECKS (do not execute from repo automation)
-- ---------------------------------------------------------------------------
-- A. SELECT column_name, is_nullable FROM information_schema.columns
--    WHERE table_schema='public' AND table_name='story_video_replies'
--      AND column_name IN ('user_id','recipient_user_id','story_id');
-- B. SELECT conname, pg_get_constraintdef(oid), confdeltype FROM pg_constraint
--    WHERE conrelid='public.story_video_replies'::regclass AND contype='f';
-- C. SELECT count(*) FROM public.story_video_replies;
-- D. SELECT count(*) FILTER (WHERE user_id IS NULL) FROM public.story_video_replies;
-- E. SELECT count(*) FILTER (WHERE recipient_user_id IS NULL) FROM public.story_video_replies;
-- F. SELECT count(*) FILTER (WHERE user_id IS NULL AND recipient_user_id IS NULL) FROM public.story_video_replies;
-- G. SELECT count(*) FROM public.story_video_replies WHERE user_id IS NOT NULL AND recipient_user_id IS NOT NULL AND user_id <> recipient_user_id;
-- H. SELECT count(*) FROM public.story_video_replies WHERE user_id IS NOT NULL AND recipient_user_id IS NOT NULL AND user_id = recipient_user_id;
-- I/J orphan auth refs via LEFT JOIN auth.users
-- K. SELECT relrowsecurity FROM pg_class WHERE relname='story_video_replies';
-- L. SELECT id, status FROM public.account_deletion_requests WHERE status='approved';
--
-- ---------------------------------------------------------------------------
-- PRODUCTION POST-MIGRATION READ-ONLY CHECKS
-- ---------------------------------------------------------------------------
-- Re-run A/B; verify user_id nullable, both auth FK confdeltype=n (SET NULL),
-- story_id FK still CASCADE; row/NULL counts unchanged; RLS policies present;
-- SELECT public.verify_account_deletion_schema_execution_ready();
-- Re-run L for approved deletion request unchanged.

BEGIN;

-- ---------------------------------------------------------------------------
-- A) story_video_replies.user_id — nullable for party-detach retention
--    Constraint: story_video_replies_user_id_fkey
...
ALTER TABLE public.story_video_replies
  DROP CONSTRAINT IF EXISTS story_video_replies_user_id_fkey;

ALTER TABLE public.story_video_replies
  ADD CONSTRAINT story_video_replies_user_id_fkey
  FOREIGN KEY (user_id) REFERENCES auth.users(id) ON DELETE SET NULL;

-- ---------------------------------------------------------------------------
-- B) story_video_replies.recipient_user_id — SET NULL auth FK
--    recipient_user_id is already nullable in Production baseline.
--    Constraint: story_video_replies_recipient_user_id_fkey
--    Current: uuid NULLABLE → auth.users(id) ON DELETE CASCADE
-- ---------------------------------------------------------------------------
ALTER TABLE public.story_video_replies
  DROP CONSTRAINT IF EXISTS story_video_replies_recipient_user_id_fkey;

ALTER TABLE public.story_video_replies
  ADD CONSTRAINT story_video_replies_recipient_user_id_fkey
...
-- ---------------------------------------------------------------------------
-- C) Extend live catalog schema-readiness probe (preserves all prior checks)
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- 6a) story_video_replies.user_id nullable + SET NULL auth FK
  check_ok := coalesce((
    SELECT c.is_nullable = 'YES'
    FROM information_schema.columns AS c
    WHERE c.table_schema = 'public'
    ...
    WHERE nsp.nspname = 'public'
      ...
      AND con.contype = 'f'
      AND pg_catalog.pg_get_constraintdef(con.oid) LIKE '%FOREIGN KEY (user_id)%REFERENCES auth.users%'
      AND con.confdeltype = 'n'
  );
  prerequisites := prerequisites || jsonb_build_array(jsonb_build_object(
    'id', 'story_video_replies_user_id_set_null',
    'satisfied', check_ok,
    'detail', 'story_video_replies.user_id nullable with ON DELETE SET NULL auth.users FK'
  ));
  all_ready := all_ready AND check_ok;

  -- 6b) story_video_replies.recipient_user_id nullable + SET NULL auth FK
  check_ok := coalesce((
    SELECT c.is_nullable = 'YES'
    FROM information_schema.columns AS c
    WHERE c.table_schema = 'public'
    ...
    WHERE nsp.nspname = 'public'
      AND rel.relname = 'story_video_replies'
      AND con.contype = 'f'
      AND pg_catalog.pg_get_constraintdef(con.oid) LIKE '%FOREIGN KEY (recipient_user_id)%REFERENCES auth.users%'
      AND con.confdeltype = 'n'
  );
  prerequisites := prerequisites || jsonb_build_array(jsonb_build_object(
    'id', 'story_video_replies_recipient_user_id_set_null',
    'satisfied', check_ok,
    'detail', 'story_video_replies.recipient_user_id nullable with ON DELETE SET NULL auth.users FK'
  ));
  all_ready := all_ready AND check_ok;
...
COMMENT ON FUNCTION public.verify_account_deletion_schema_execution_ready() IS
  ...
  'Includes story_video_replies auth-user FK SET NULL prerequisites (Phase 4C.7B.1E.2B.2). '
